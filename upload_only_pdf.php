...
<?php 	
        ...
        $uniq_session = htmlspecialchars($_SESSION['uniq_identifie'], ENT_QUOTES, 'UTF-8') . '.pdf';

        if (!in_array($video_ext, $allowed_ext)) {
            $error_message =  "Only PDF are allowed";
        }
        ...
        if (!isset($error_message)) {
            ...
            try {                
                ...
                $error_message = 'sorry pdf not uploaded, please try again';
            } catch (Exception $e) {
                $error_message = 'sorry pdf not uploaded, please try again';
            }           
        }
        ...
        if (!isset($error_message)) {
            $insert = mysqli_query($connection, "INSERT INTO rev_base64(base64_img_text,uniq_url,rev_uniq_session,rev_uploaded_on,rev_user_id) VALUES ('0', '$uniq_session', '$uniq_session', '$time_and_date', '$user_name')");        
        }  
?>