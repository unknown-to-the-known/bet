			...
			<div class="d-flex justify-content-around">
				...
				<!-- <button type="button" class="btn btn-outline-primary btn-sm">
				  Total Books fee Rs. <?php 
				  	$sqls_book = "SELECT SUM(rev_amount) AS total_amounts_book FROM erp_bill WHERE rev_sts = '1' AND rev_paid_to = 'rev_books' AND rev_paid_on BETWEEN '$st_date' AND '$et_date'";
				...
				<button type="button" class="btn btn-outline-primary btn-sm">
				  Total Transportation fee Rs. <?php 
				  	$sqls_trans = "SELECT SUM(rev_amount) AS total_amounts_trans FROM erp_bill WHERE rev_sts = '1' AND rev_paid_to = 'rev_trans' AND rev_paid_on BETWEEN '$st_date' AND '$et_date'";
				...
				<!-- <button type="button" class="btn btn-outline-primary btn-sm">
				  Total Discount Rs. <?php 
				  	echo $sqls_discount = "SELECT SUM(rev_discount) AS total_amounts_discount FROM erp_bill WHERE rev_paid_on BETWEEN '$st_date' AND '$et_date' AND rev_sts = '1'";
			  	...
			  	<?php 
			  		...
			  		if (mysqli_num_rows($fetch_all_active_bills) > 0) {
			  			...
			  			while($rs = mysqli_fetch_assoc($fetch_all_active_bills)) { ?>
						      ...
						      <td>
						      	<?php 
										...
										if (mysqli_num_rows($fetch_student_name) > 0) {
										    $d = mysqli_fetch_assoc($fetch_student_name);

										    $student_f_name = $d['rev_student_fname'];
										    $student_m_name = ($d['rev_student_mname'] != '0') ? $d['rev_student_mname'] : '';
										    $student_l_name = ($d['rev_student_lname'] != '0') ? $d['rev_student_lname'] : '';

										    $father_f_name = $d['rev_father_fname'];
										    $father_m_name = ($d['rev_father_mname'] != '0') ? $d['rev_father_mname'] : '';
										    $father_l_name = ($d['rev_father_lname'] != '0') ? $d['rev_father_lname'] : '';

										    $student_class = $d['rev_admission_class'];
										    $student_tution_fee = $d['rev_fees'];
										    $student_sdf = $d['rev_admission_fee'];
										}

										// Combine student full name
										$student_full_name = trim("$student_f_name $student_m_name $student_l_name");

										// Combine father full name (if needed later)
										$father_full_name = trim("$father_f_name $father_m_name $father_l_name");
						    	...
						    	<td><?php echo $father_full_name; ?></td>
						      	...
						      	<?php 
						      		if (in_array($student_class, ['baby', 'lkg', 'ukg', '1', '2', '3', '4', '5', '6', '7', '8', '9'])) {
										    $student_term_fee = '2000';
										} elseif ($student_class == '10') {
										    $student_term_fee = '3000';
										}
						      	?>

						      	<!-- <td>Rs. <?php echo $student_term_fee; ?></td> -->
						      	<!-- <td>Rs. <?php echo $student_sdf; ?></td> -->
						      	<td>Rs. <?php echo $student_tution_fee; ?></td>
						      <td><?php echo $student_class; ?></td>
						      <td><?php echo htmlspecialchars($paid_to, ENT_QUOTES, 'UTF-8'); ?></td>
						      <td><?php echo htmlspecialchars($rs['erp_mode'], ENT_QUOTES, 'UTF-8'); ?></td>
						      ...
						      <td><?php echo htmlspecialchars($rs['collected_by'], ENT_QUOTES, 'UTF-8'); ?></td>
