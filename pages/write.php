...
?>
<?php 
$ses = htmlspecialchars($_GET['uni'], ENT_QUOTES, 'UTF-8');
       $check_if_uniqi_id_active = mysqli_query($connection,"SELECT * FROM rev_homework WHERE rev_uniq_identifier = '$ses' AND rev_hw_sts = '1'");
      if (mysqli_num_rows($check_if_uniqi_id_active) > 0) {
        $date_of_uplading = date('Y-m-d H:i:s a');
        $uniq_url_of_uploading = md5($date_of_uplading . '_' . $teacher_email_id);
        header("Location: https://teacher.amazing-feynman.3-108-84-48.plesk.page/pages/create_homework?param=" . $class_id . "&uni=" . $uniq_url_of_uploading);
      }
?>

<?php 
    if (isset($_POST['delete'])) {
        $del_id = mysqli_escape_string($connection, trim($_POST['delete_id']));
        ...
        if (!isset($error_message)) {
            $check_if_id_belongs_to_same_teacher = mysqli_query($connection, "SELECT * FROM rev_homework WHERE tree_id = '$del_id' AND rev_teacher_id = '$teacher_email_id' AND rev_hw_sts = '1'");

            if (mysqli_num_rows($check_if_id_belongs_to_same_teacher) > 0) {
                $update = mysqli_query($connection, "UPDATE rev_homework SET rev_hw_sts = '0' WHERE tree_id = '$del_id' AND rev_teacher_id = '$teacher_email_id'");
            }
        }
    }
?>
...
Main Banner START -->
    ...
    <!-- Svg decoration -->
    <figure class="position-absolute top-100 end-0 translate-middle-y me-n8 d-none d-sm-block mt-8">
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-8 mb-2 mb-lg-4 position-relative">
                ...
                <!-- Title -->
                <h1 class="display-4 mb-3 mb-lg-0 text-dark" style="font-size: 40px;">Write</h1>
            </div>

            <!-- Info and buttons -->
            <div class="col-md-8 mb-4 mb-lg-0">

                <div class="mt-4 up_box">

                <div class="card card-body shadow p-4 p-sm-5 position-relative">

                    <h4 class="text-center text-info fw-bold mb-4 d-flex justify-content-center" style="display: flex;">
                        <div class="icon-lg fs-4 text-white bg-white bg-opacity-10 rounded flex-shrink-0 up_click"> 
                            <img src="../assets/images/write.webp" class="rounded-3" alt="read" width="100%" height="100%">
                        </div><span class="mt-2">&nbsp;&nbsp;Create Writing Activity</span>
                    </h4>
                    <div class="col-md-12 d-flex justify-content-center">
                    <div class="alert alert-danger text-center alert-dismissible fade show error_message" role="alert" style="display: inline-flex;">                   
                        ...
                        <div class="mt-2" style="font-size: 14px">&nbsp;&nbsp;Sorry! Activity is already scheduled for today</div>
                        </div>
                    </div>

                    <div class="col-md-12 d-flex justify-content-center">
                <div class="alert alert-success text-center alert-dismissible fade show success_message" role="alert" style="display: inline-flex;">                    
                    <ul class="feedback d-flex justify-content-center">
                    ...
                    <div class="mt-2">&nbsp;&nbsp;Success, Activity created, Please wait for 2 seconds</div>
                     
                </div>
                </div>

                <div class="col-md-12 d-flex justify-content-center">
                    <div class="alert alert-warning text-center alert-dismissible fade show empty" role="alert" style="display: inline-flex;">
                        <ul class="feedback d-flex justify-content-center">
                        <li class="ok active">
                            <div></div>
                        </li>
                        </ul>   
                        <div class="mt-2" style="font-size: 14px">&nbsp;&nbsp;Please fill all the fields</div>                      
                    </div>
                </div>

                    <!-- Form START -->
                    <form class="row g-3 position-relative" id="mkform"  method="post" enctype="multipart/form-data">
                        <!-- Name -->
                        <div class="col-md-6 col-lg-12 col-xl-6">
                            <div class="bg-body shadow rounded-pill p-2">
                                <div class="input-group">
                                    <input class="form-control border-0 me-1 homework_name" type="text" placeholder="Activity name*" name="homework_name" required autocomplete="off">
                        ...
                        <div class="col-md-6 col-lg-12 col-xl-6">
                            <div class="bg-body shadow rounded-pill p-2">
                                <div class="input-group">
                                    <input class="form-control border-0 me-1 dates" type="text" placeholder="Date*" name="date"  id="datepickers" required autocomplete="off" value="<?php echo date('d-M-Y'); ?>">
                        ...
                        <div class="col-md-6 col-lg-12 col-xl-6">
                            <div class="bg-body shadow rounded-pill p-2">
                                <div class="input-group">
                                    <input class="form-control border-0 me-1 start_time" type="text" placeholder="Start time*" name="start" id="timepicker" required autocomplete="off" value="<?php echo date('h:i a'); ?>">
                        ...
                        <div class="col-md-6 col-lg-12 col-xl-6">
                            <div class="bg-body shadow rounded-pill p-2">
                                <div class="input-group">
                                    <input class="form-control border-0 me-1 end_time" type="text" name="end" id="timepicker_2" placeholder="End time*" required autocomplete="off" value="<?php echo date('h:i a', strtotime('+1 hour')); ?>">
                                    <p style="font-size: 20px;">⌛</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="shadow-lg p-2 mb-3 bg-body rounded col-md-4">
                                <div class="d-flex justify-content-center" style="display: inline-flex;">
                                    ...
                                    <div class="avatar avatar-sm me-2 rounded-4">
                                        <img class="avatar-img rounded-1" src="../assets/images/ModeofHW.webp" alt="Homework paper">
                                    </div>
                                    <!-- Avatar info -->
                                    <div>
                                        <h6 class="mb-0 text-dark mt-2">Mode of write activity*</h6>
                                    </div>
                                </div>
                            <div class="d-flex justify-content-center mt-2 fw-bold">
                                <div class="form-check form-check-inline text">
                                  <input class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio1" value="text">
                                  <label class="form-check-label" for="inlineRadio1">Text</label>
                                </div>
                                <div class="form-check form-check-inline img">
                                ...
                                <div class="form-check form-check-inline pdf">
                                  <input class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio3" value="pdf">
                                  <label class="form-check-label" for="inlineRadio3">PDF</label>
                                </div>
                            </div>
                            </div>
                        </div>
                ...
                <div class="uploader1"> 
                    ...
                    <input type="hidden" role="uploadcare-uploader"  data-public-key="ed6ea2c691a873422dc0"
                        data-tabs="file" data-input-accept-types="application/pdf" class="pd" name="pdf" />
                    <button class="btn btn-primary submit_pdf" name="pdf_upload" type="button">Submit</button>                  
                </div>
                <!-- PDF  -->
            ...
            <!-- Image Uploader -->
                <div class="uploader2">
                    ...
                    <div class="col-md-6">
                      ...
                      <input class="form-control form-control-lg" id="file" type="file" accept="image/*" onchange="preview_image()">
                      <div class="mt-2 col-md-12 d-flex justify-content-center">
                      <button class="loader">
                          <i class="fa fa-spinner fa-spin" style="font-size: 24px"></i>Please wait while we are uploading your image
                      </button>
                      </div>

                    </div>

                <!-- <input name="ques_pdf" type="file" role="uploadcare-uploaders"/> -->
                            <br>
                            ...
                            <?php 
                                if (isset($uniq_id)) {
                                     $mnj = $uniq_id;
                                    $fetch_active_images = mysqli_query($connection, "SELECT * FROM rev_base64 WHERE rev_uniq_session = '$mnj'");

                                    if (mysqli_num_rows($fetch_active_images) > 0) {
                                        while($hgt = mysqli_fetch_assoc($fetch_active_images)) { ?>
                                            <div class="col-md-4" style=" margin-top:5px">
                                                <div class="shadow-lg bg-body rounded" style='width:100%'>
                                                    <div class="card-body">
                                                        <button class="btn btn-danger delete" type="button" style="float:right" onClick='GFG_click(this.id)' id="<?php echo htmlspecialchars($hgt['uniq_url'], ENT_QUOTES, 'UTF-8'); ?>">Delete</button>
                                                        <a href="https://d349p3fjrxa3i.cloudfront.net/<?php echo htmlspecialchars($hgt['uniq_url'], ENT_QUOTES, 'UTF-8'); ?>" target="_blank"><img src="https://d349p3fjrxa3i.cloudfront.net/<?php echo htmlspecialchars($hgt['uniq_url'], ENT_QUOTES, 'UTF-8'); ?>"></a>
                                                    </div>
                                                </div>
                                            </div>
                                        <?php }
                            ...
                            <div class="col-md-12 d-flex justify-content-center">
                                ...
                                <a href="#scroll_up"><button type="button" class="btn btn-primary mb-0 submit_image" name="submit" style="margin-top:10px;">Submit</button></a>
                            </div>
                     
                 </div>

                        <div class="form-outline uploader3">
                            <div class="alert alert-warning text-center" role="alert" style="font-size:20px;">
                              Do not refresh the page
                            </div>                          
                          <svg xmlns="http://www.w3.org/2000/svg" style="display: none;">
                            ...
                            <div class="paper mt-2 mb-4" style="margin: 0 auto;">
                                <div class="paper-content" style="justify-content: left; ">
                                    <textarea autofocus class="text_area"></textarea>
                                </div>
                            </div>
    ...
    <?php 
        $fetch_hw_query = mysqli_query($connection, "SELECT * FROM rev_lsrw WHERE   rev_class = '$subject_class_yt' AND rev_sec = '$class_sec' AND  rev_sch = '$user_school' AND rev_teacher_id = '$teacher_email_id' AND  rev_sts = '1' AND rev_category = 'text_write' OR rev_category = 'text_write_image' OR rev_category = 'text_write_pdf'");
        if (mysqli_num_rows($fetch_hw_query) > 0) { ?>

<div class="container">
    <h3 class="mt-5 mb-4 text-center"><img src="<?php echo BASE_URL; ?>assets/images/write.webp" width="40px" height="40px" alt="Write">&nbsp;Activity List</h3>
    <div id="table-scroll" class="table-scroll">
      <div class="table-wrap">
        <table class="main-table">
          <thead class="new_data">
            <tr class="table_header text-center text_highlight">
              ...
              <th class="fixed-side border-0" style="background: #fff; color: #0CBC87" scope="col"><span class="badge bg-primary bg-opacity-10 text-primary" style="font-size: 16px; font-weight: bold">Activity name</span></th>
              ...
              <th scope="col" class="border-0"><span class="badge bg-primary bg-opacity-10 text-primary" style="font-size: 16px; font-weight: bold">Schedule</span></th>
          ...
          <tbody>
            <?php 
                ...
                if (mysqli_num_rows($fetch_hw_query) > 0) {
                    $i = 1;
                    while($ju = mysqli_fetch_assoc($fetch_hw_query)) { ?>
                        <tr class="text-center" style="font-size: 18px">
                          <th class="fixed-side"><?php echo $i++; ?></th>
                          <th class="fixed-side" style="text-align: left; word-wrap: break-word; white-space: normal !important;"><img src="<?php echo BASE_URL; ?>assets/images/write.webp" width="25px" height="25px" alt="write"><a href="<?php echo BASE_URL; ?>pages/lsrw_write?id=<?php echo htmlspecialchars($ju['tree_id'], ENT_QUOTES, 'UTF-8'); ?>" style="text-decoration: none;">&nbsp;<?php echo htmlspecialchars(ucfirst($ju['rev_name']), ENT_QUOTES, 'UTF-8'); ?></a></th>
                          <td><?php echo htmlspecialchars(ucfirst($ju['rev_lsrw_sub']), ENT_QUOTES, 'UTF-8'); ?></td>
                          <td><img src="<?php echo BASE_URL; ?>assets/images/calendar.webp" width="20px" height="20px" alt="Calendar">&nbsp;<?php echo htmlspecialchars(date('d-M-Y', strtotime($ju['rev_date'])), ENT_QUOTES, 'UTF-8'); ?>&nbsp;<br><?php echo htmlspecialchars(date('h:i a', strtotime($ju['rev_start_time'])), ENT_QUOTES, 'UTF-8'); ?> to <?php echo htmlspecialchars(date('h:i a', strtotime($ju['rev_end_time'])), ENT_QUOTES, 'UTF-8'); ?></td>
                              <td>

                                <button href="#" class="btn btn-sm btn-success-soft btn-round me-1 mb-0" data-bs-toggle="modal" data-bs-target="#staticBackdrop1" data-bs-whatever-edit-name="<?php echo htmlspecialchars($ju['rev_name'], ENT_QUOTES, 'UTF-8'); ?>" data-bs-whatever-edit-date="<?php echo htmlspecialchars($ju['rev_date'], ENT_QUOTES, 'UTF-8'); ?>" data-bs-whatever-edit-start-time="<?php echo htmlspecialchars($ju['rev_start_time'], ENT_QUOTES, 'UTF-8'); ?>" data-bs-whatever-edit-end-time="<?php echo htmlspecialchars($ju['rev_end_time'], ENT_QUOTES, 'UTF-8'); ?>" data-bs-whatever-edit-id="<?php echo htmlspecialchars($ju['tree_id'], ENT_QUOTES, 'UTF-8'); ?>"><i class="far fa-fw fa-edit"></i>
                                </button>
                                <button class="btn btn-sm btn-danger-soft btn-round mb-0" data-bs-toggle="modal" data-bs-target="#staticBackdrop2" data-bs-whatever-delete-name="<?php echo htmlspecialchars($ju['rev_name'], ENT_QUOTES, 'UTF-8'); ?>" data-bs-whatever-delete-id="<?php echo htmlspecialchars($ju['tree_id'], ENT_QUOTES, 'UTF-8'); ?>"><i class="fas fa-fw fa-times"></i></button>
                              </td>           
                        </tr>   
                        <?php }                    
                        }
        ...
        <div class="container">
            <div class="w-100 mt-auto d-inline-flex justify-content-center">
                <div class="shadow-lg p-2 mb-3 bg-body rounded d-flex align-items-center">
                    <!-- Avatar -->
                    <div class="avatar avatar-sm me-2 rounded-4">
                        <img class="avatar-img rounded-1" src="../assets/images/read.webp" alt="avatar">
                    </div>
                    <!-- Avatar info -->
                    <div>
            ...
            <form class="row g-3 position-relative">
                        ...
                        <div class="col-md-6 col-lg-12 col-xl-6">
                            <div class="bg-body shadow rounded-pill p-2">
                                <div class="input-group">
                                    <input class="form-control border-0 me-1 st_name" type="text" placeholder="Audio name*" required>
                                </div>
                            </div>
                        </div>
                        <!--Date -->
                        <div class="col-md-6 col-lg-12 col-xl-6">
                            <div class="bg-body shadow rounded-pill p-2">
                                <div class="input-group">
                                      <input type="date" class="form-control border-0 me-1 hm_da da st_date" placeholder="Audio Date"  name="date_up" min="<?php echo date('Y-m-d'); ?>"/>
                                </div>
                            </div>
                        </div>
                        <!-- Start time -->
                        <div class="col-md-6 col-lg-12 col-xl-6">
                            <div class="bg-body shadow rounded-pill p-2">
                                <div class="input-group">
                                    <input class="form-control border-0 me-1 time_picker st_start_time" type="text" placeholder="Start time*" name="start" id="timepicker3" required>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6 col-lg-12 col-xl-6">
                            <div class="bg-body shadow rounded-pill p-2">
                                <div class="input-group">
                                    <input class="form-control border-0 me-1 time_picker st_end_time" type="text" name="end" id="timepicker4" placeholder="End time*" required>
                                </div>
                            </div>
                        </div>
                        <!-- Button -->
                        <div class="col-md-12 mb-4 d-flex justify-content-center">
                            <input type="text" name="edit_id" class="st_id" value="">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="w-100 mt-auto d-inline-flex justify-content-center">
            <div class="shadow-lg p-2 mb-3 bg-body rounded d-flex align-items-center">
                <!-- Avatar -->
                <div class="avatar avatar-sm me-2 rounded-4">
                    <img class="avatar-img rounded-1" src="../assets/images/read.webp" alt="read">
                </div>
                <!-- Avatar info -->
                <div>
      ...
      <div class="modal-footer">
        <form action="" method="post">
            <input type="text" name="delete_id" class="st_id_delete" value="">
...
// For text
    $('.submit').click(function() {
        var home_name = $('.homework_name').val();
        var date = $('.dates').val();
        var start_time = $('.start_time').val();
        var end_time = $('.end_time').val();
        var text_area = $('.text_area').val();

        if (home_name == "" ||  date == '1970-01-01' || start_time == "00:00" || end_time == "00:00") {
            $(".empty").fadeTo(5000, 500).slideUp(500, function(){
                $(".empty").slideUp(500);
            });
        } else {
            
                $.post("text_write.php", { name: home_name, class: "<?php echo $subject_class_yt; ?>", sec: "<?php echo $class_sec; ?>", subject: "<?php echo $subject_name_yt; ?>", school: "<?php echo $user_school; ?>", date: date, start_time: start_time, end_time: end_time, format: "text_write",  uniq_identifier: "<?php echo $uniq_identifier; ?>", text:text_area }).done(function( text ) {
                        if ($.isNumeric(text)) {
                            var ls = $.trim(text);                  
                            $('.new_data').append('<tr class="text-center text_highlight" style="font-size: 18px;"><th class="fixed-side">New</th><th class="fixed-side" style="text-align: left; max-width: 5px"><img src="<?php echo BASE_URL; ?>assets/images/Homework.webp" width="25px" height="25px" alt="Homework"><a href="<?php echo BASE_URL; ?>pages/full_details_lsrw?id=' + ls + '" style="text-decoration: none;">&nbsp;' + home_name + '</a></th><td><?php echo $subject_name_yt; ?></td><td><img src="<?php echo BASE_URL; ?>assets/images/calendar.webp" width="20px" height="20px" alt="Calendar">&nbsp;' + date +'&nbsp;<br>' + start_time + ' to ' + end_time + '</td></tr>');
                            $('#mkform')[0].reset();
                            ...
                            $(".error_message").fadeTo(5000, 500).slideUp(500, function(){
...
// FOR Image
    $('.submit_image').click(function() {
        ...
        if (home_name == "" ||   date == '1970-01-01' || start_time == "00:00" || end_time == "00:00") {
            ...
            var text_area = $('.text_area').val();          
                $.post( "text_img_write.php", { name: home_name, class: "<?php echo $subject_class_yt; ?>", sec: "<?php echo $class_sec; ?>", subject: "<?php echo $subject_name_yt; ?>", school: "<?php echo $user_school; ?>", date: date, start_time: start_time, end_time: end_time, format: "text_write_image", uniq_identifier:"<?php echo htmlspecialchars($_GET['uni'], ENT_QUOTES, 'UTF-8'); ?>"}).done(function( text ) {
                        console.log(text);
                        if ($.isNumeric(text)) {
    ...
    let resized_base64 = await new Promise((resolve) => {
        ...
        img.onload = () => {
            ...
            var k = canvas.toDataURL(); 
                $.post( "../imag_up.php", { name: k, uniq_identif: "<?php echo htmlspecialchars($_GET['uni'], ENT_QUOTES, 'UTF-8'); ?>", sch:"<?php echo $user_school; ?>",cla:"<?php echo $subject_class_yt; ?>",sec:"<?php echo $class_sec; ?>",sub:"<?php echo $subject_name_yt; ?>" })
                  .done(function( data ) {
                    var b = $.trim(data);
                    if (b != "") {
                        $('.imge').append("<div class='col-md-4 hhj' style='margin-left:10px; margin-top:5px'><div class='shadow-lg bg-body rounded' style='width:100%'><div class='card-body'><button class='btn btn-danger delete' type='button' style='float:right' id='"+b+"' onClick='GFG_click(this.id)''>Delete</button><a href='https://d349p3fjrxa3i.cloudfront.net/"+b+"' target='_blank'><img src='https://d349p3fjrxa3i.cloudfront.net/"+b+"'></a></div></div></div>")
                        // $(".loader").hide();
                        $(".loader").fadeTo(500, 500).slideUp(500, function(){
                            $(".loader").slideUp(500);
                        });
                        $(".submit").show();
                    }                    
...
$('.submit_pdf').click(function() {     
        ...
        var start_time = $('.start_time').val();
        var end_time = $('.end_time').val();
        var text_area = $('.pd').val();

        if (home_name == "" ||  text_area == "" || date == '1970-01-01' || start_time == "00:00" || end_time == "00:00") {
            $(".empty").fadeTo(5000, 500).slideUp(500, function(){
                $(".empty").slideUp(500);
            });
         } else {           
            if (text_area != "") {
                $.post( "text_write_pdf.php", { name: home_name, class: "<?php echo $subject_class_yt; ?>", sec: "<?php echo $class_sec; ?>", subject: "<?php echo $subject_name_yt; ?>", school: "<?php echo $user_school; ?>", date: date, start_time: start_time, end_time: end_time, format: "text_write_pdf", text: text_area}).done(function( text ) {
                        if ($.isNumeric(text)) {
                            var ls = $.trim(text);  
                            $('.new_data').append('<tr class="text-center text_highlight" style="font-size: 18px;"><th class="fixed-side">New</th><th class="fixed-side" style="text-align: left; max-width: 5px"><img src="<?php echo BASE_URL; ?>assets/images/Homework.webp" width="25px" height="25px" alt="Homework"><a href="<?php echo BASE_URL; ?>pages/full_details_lsrw?id=' + ls + '" style="text-decoration: none;">&nbsp;' + home_name + '</a></th><td><?php echo $subject_name_yt; ?></td><td><img src="<?php echo BASE_URL; ?>assets/images/calendar.webp" width="20px" height="20px" alt="Calendar">&nbsp;' + date +'&nbsp;<br>' + start_time + ' to ' + end_time + '</td></tr>');
                            $('#mkform')[0].reset();
