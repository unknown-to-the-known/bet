...
<?php 
    ...
    $_SESSION['uniq_id'] = $uniq_id = md5($date_of_uplading . '_' . $teacher_email_id);
?>

<?php 
        if (isset($_GET['param'])) {
            if ($_GET['param'] != "") {
                $teacher_changed_class = htmlspecialchars($_GET['param'], ENT_QUOTES, 'UTF-8');
                $fetch_class_of_teacher = mysqli_query($connection, "SELECT * FROM rev_teach_class WHERE rev_teach_email_id = '$teacher_email_id' AND rev_school_uniq_id = '$school_id' AND rev_teach_sts = '1' AND tree_id = '$teacher_changed_class'");

            if (mysqli_num_rows($fetch_class_of_teacher) > 0) {
                while($fdr = mysqli_fetch_assoc($fetch_class_of_teacher)) { 
                    $class_default = $fdr['rev_teacher_class'] . ' ' . ucfirst($fdr['rev_teacher_sec']) . ', ' . ucfirst(substr($fdr['rev_teach_subject'], 0, 3));
                    $class_id = $fdr['tree_id'];

                   $teacher_class = $_SESSION['teacher_class'] = $fdr['rev_teacher_class'];
                    $_SESSION['teacher_sec'] = $fdr['rev_teacher_sec'];
                    $_SESSION['teacher_subject'] = $fdr['rev_teach_subject'];
                }
            } else {
                header("Location: " . BASE_URL . 'pages/action');
            }
...
// $url = 'https://api.greenapi.com/waInstance1101805411/sendMessage/01861500b865440090f89bef652839bd70ca20c64b1f4dc7b7';

// //chatId is the number to send the message to (@c.us for private chats, @g.us for group chats)
// $data = array(
//     'chatId' => '919164454002@c.us 917829103781@c.us',
//     'message' => 'Hello Worldsdss'
// );
        ...
        if (!isset($error_message)) {
            ...
            if (mysqli_num_rows($check_if_id_belong_to_the_same_teacher) > 0) {
                ...
                if (isset($delete_lsrw_data)) {
                    $error_message = "Success, Listen activity deleted";
                }
            }
        }
    }
            ...
            if (mysqli_num_rows($fetch_id_belong_to_same_teacher) > 0) {
                $check_if_there_is_a_clash = mysqli_query($connection, "SELECT * FROM rev_lsrw WHERE rev_date = '$convert_edit_date' AND rev_teacher_id = '$teacher_email_id' AND rev_sts = '1' AND tree_id != '$audi_edit_id'");
                    ...
                    if(mysqli_num_rows($check_if_there_is_a_clash) > 0) {
                        $error_message = "Homework already present for the selected date";
                    }
                    if (!isset($error_message)) {
                        ...
                        if (isset($update_edit)) {
                            $success_message = "Success, Listen activity updated";
                        }
                    }
...
<?php 
    $fetch_issue = mysqli_query($connection, "SELECT * FROM issue_box");

    if (mysqli_num_rows($fetch_issue) > 0) {
        while($iss = mysqli_fetch_assoc($fetch_issue)) { ?>
            <div class="alert alert-danger alert-dismissible fade show mt-2 mb-0 rounded-3 fw-bold" role="alert">
                <!-- Avatar -->
                <div class="avatar avatar-xs me-2">
                    <img class="avatar-img rounded-circle" src="https://rev-user.s3.ap-south-1.amazonaws.com/Revisewell_logo.svg" alt="avatar">
                </div>
                <!-- Info -->
                There is a regular maintainance of the website on 29-05-2024 from 10 AM to 5 PM
...
<style type="text/css">
    ...
    <p class="float-end" style=" border-bottom: 1px dotted; color: #000; font-family: 'Nunito Sans', sans-serif; cursor: pointer;" data-bs-toggle="modal" data-bs-target="#class_selector" title="change class">Grade <?php echo $class_default; ?><i class="fas fa-caret-down"></i></p>    
</div>
    ...
    <div class="container" style="margin-top:-50px;">
        <div class="row">
            ...
            <div class="col-md-8 mb-2 mb-lg-4 position-relative">
                ...
                <!-- Title -->
                <!-- <div class="icon-lg fs-4 rounded flex-shrink-0" data-bs-toggle="modal" data-bs-target="#staticBackdrop3" style="cursor: pointer; float: right;">  -->
                    <!-- <img src="../assets/images/speech-bubble2.gif" /> -->
                    <!-- <iframe src="https://embed.lottiefiles.com/animation/4975" width="80%" height="80%"></iframe> -->
                <!-- </div> -->
                <!-- <h1 class="display-4 mb-3 mb-lg-0 text-dark" style="font-size: 40px;">Listen</h1> -->
            ...
            <div class="col-md-8 mb-4 mb-lg-0">
                ...
                <div class="tab-pane fade show active" id="course-pills-tab1" role="tabpanel" aria-labelledby="course-pills-tab-1">
                    ...
                    <h4 class="text-center text-info fw-bold mb-4">Create listen activity</h4>
                        <div class="row g-3 position-relative">
                         <div class="col-md-6 col-lg-12 col-xl-6">
                            <div class="bg-body shadow rounded-pill p-2">
                                <div class="input-group">
                                    <input class="form-control border-0 me-1 st_date" type="text" placeholder="Date*" name="date"  id="datepicker_up" required autocomplete="off">
                        ...
                        <div class="col-md-6 col-lg-12 col-xl-6">
                            <div class="bg-body shadow rounded-pill p-2">
                                <div class="input-group">
                                    <input class="form-control border-0 me-1 st_audio" type="text" placeholder="Enter audio name*" name="audio_name" autocomplete="off" required autocomplete="off">
                                    <p style="font-size: 20px;"><img>🔊</p>
                                </div>
                            </div>
                        </div>
                        
                        <div class="uploader2" style="margin-top:10px; text-align: center;">
                            <div class="col-md-12">
                              <button id="uppyModalOpener" class="btn btn-danger-soft btn-sm d_up c_btn upp">Record or Upload</button>
                              <button class="btn btn-danger-soft btn-sm d_up c_btn yt">Youtube Video</button>
                        ...
                        <!-- Button -->
                            ...
                            <!-- <input type="hidden" name="youtube_uniq_id" value="<?php echo $uniq_id; ?>"> -->
                            <input class="form-control form-control-lg youtube_link" type="text" placeholder="YouTube Video Link" aria-label=".form-control-lg example" name="youtube_link" autocomplete="off">
                            <div class="col-md-12 d-flex justify-content-center">
                                <button type="submit" class="btn btn-info mb-0 submit" name="submit" id="liveToastBtn">Submit</button>
                            </div>
            ...
            <!-- Image -->
            <div class="col-md-4 desktop_show">
    ...
    <div id="table-scroll" class="table-scroll">
      <div class="table-wrap">
        <table class="main-table">
          ...
          <tbody>
            <?php 
                $fetch_audio_file = mysqli_query($connection, "SELECT * FROM rev_lsrw WHERE rev_teacher_id = '$teacher_email_id' AND rev_sts = '1'  AND rev_category = 'listen' AND rev_school_id = '$school_id' AND rev_class = '$teacher_class' ORDER BY tree_id DESC");
                if (mysqli_num_rows($fetch_audio_file) > 0) {
                    ...
                    while($rol = mysqli_fetch_assoc($fetch_audio_file)) { ?>
                        <tr class="text-center" style="font-size: 18px">
                          ...
                          <th class="fixed-side" style="text-align: left; word-wrap: break-word; white-space: normal !important"><img src="<?php echo BASE_URL; ?>assets/images/listen.webp" width="25px" height="25px" alt="Listen">&nbsp;<a href="<?php echo BASE_URL; ?>pages/listen_audio?id=<?php echo htmlspecialchars($rol['tree_id'], ENT_QUOTES, 'UTF-8'); ?>&uni=<?php echo htmlspecialchars($rol['rev_uniq_link'], ENT_QUOTES, 'UTF-8'); ?>" style="text-decoration: none;"><?php echo htmlspecialchars(ucfirst($rol['rev_name']), ENT_QUOTES, 'UTF-8'); ?></a></th>
                          ...
                          <td>
                            <?php 
                                if ($rol['rev_date'] >= $today_date) { ?>
                                      <button href="#" class="btn btn-sm btn-success-soft btn-round me-1 mb-0" data-bs-toggle="modal" data-bs-target="#staticBackdrop1" data-bs-whatever-edit-name="<?php echo htmlspecialchars($rol['rev_name'], ENT_QUOTES, 'UTF-8'); ?>" data-bs-whatever-edit-date="<?php echo htmlspecialchars($rol['rev_date'], ENT_QUOTES, 'UTF-8'); ?>" data-bs-whatever-edit-start-time="<?php echo htmlspecialchars($rol['rev_start_time'], ENT_QUOTES, 'UTF-8'); ?>" data-bs-whatever-edit-end-time="<?php echo htmlspecialchars($rol['rev_end_time'], ENT_QUOTES, 'UTF-8'); ?>" data-bs-whatever-edit-id="<?php echo htmlspecialchars($rol['tree_id'], ENT_QUOTES, 'UTF-8'); ?>"><i class="far fa-fw fa-edit"></i></button>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="container">
            <div class="w-100 mt-auto d-inline-flex justify-content-center">
                <div class="shadow-lg p-2 mb-3 bg-body rounded d-flex align-items-center">
                    ...
                    <div class="avatar avatar-sm me-2 rounded-4">
                        <img class="mb-2" src="../assets/images/technical-service.svg" alt="technical-support" style="width: 100%; height: 100%">
                    </div>
                    <!-- Avatar info -->
                    <div>
                        <h6 class="mb-0 text-dark modal-title">Instructions - <span class="text-primary hwname"></span></h6>
                    </div>
                </div>
            </div>
            ...
            <div class="w-100 mt-auto d-inline-flex justify-content-center">
                <div class="shadow-lg p-2 mb-3 bg-purple bg-opacity-10 rounded d-flex align-items-center btn-transition">
                <h6 class="text-danger" style="text-align: justify;">
                    <span class="badge bg-success text-white rounded-pill me-1">Step 1 :</span>Create a listen activity<br><br>
                    <span class="badge bg-success text-white rounded-pill me-1">Step 2 :</span>Go to the Audio list, check for the activity you created in <span class="badge bg-success text-white rounded-pill me-1">Step 1</span> and click on it<br><br>
                    <span class="badge bg-success text-white rounded-pill me-1">Step 3 :</span>The corresponding activity is now opened. Scroll down and create MCQs with respect to the activity created
                   </h6>
                </div>
            </div>
        ...
        <div class="container">
            <div class="w-100 mt-auto d-inline-flex justify-content-center">
                <div class="shadow-lg p-2 mb-3 bg-body rounded d-flex align-items-center">
                    ...
                    <!-- Avatar info -->
                    <div>
                        <h6 class="mb-0 text-dark modal-title">Update audio details of <span class="text-primary hwname"></span></h6>
                    </div>
                </div>
            </div>
            ...
            <form class="row g-3 position-relative" action="" method="post">
                        ...
                        <div class="col-md-6 col-lg-12 col-xl-6">
                            <div class="bg-body shadow rounded-pill p-2">
                                <div class="input-group">
                                      <input type="date" class="form-control border-0 me-1 hm_da da st_date" placeholder="Audio Date"  name="audio_edit_date" id="datepickers_t" min="<?php echo date('Y-m-d'); ?>"/>
                                </div>
                            </div>
                        </div>
                        <!-- Start time -->
      ...
      <div class="modal-body">
        ...
        <div class="w-100 mt-auto d-inline-flex justify-content-center">
            <div class="shadow-lg p-2 mb-3 bg-body rounded d-flex align-items-center">
                <!-- Avatar -->
                <div class="avatar avatar-sm me-2 rounded-4">
                    <img class="avatar-img rounded-1" src="../assets/images/listen.webp" alt="listen">
                </div>
                <!-- Avatar info -->
                <div>
...
<script type="text/javascript">
    ...
    $('.submit').click(function() {
        $('.submit').hide();
        var selected_date = $('.st_date').val();
        var selected_name = $('.st_audio').val();
        var selected_youtube = $('.youtube_link').val(); 



        var selected_date_length = selected_date.length; 
        var selected_name_length = selected_name.length;      

        if (selected_name_length === 0 || selected_date_length === 0) {
            $('.e_message').html("Please fill all the fields");
            $('.submit').show();
        } else {
            $('.toast-container').hide();
            $.post( "create_listen_activity.php", { create_date: selected_date, create_name: selected_name, create_uniq_id: "<?php echo $uniq_id; ?>", created_youtube_id:selected_youtube}).done(function( text ) {
                $('.e_message').html(text);
                ...
                if (text.trim() === 'success') {
                   ...
                   $('.activity_name').html(selected_name);

                    // $('#').show();

                    setTimeout(function(){
                    $('#create_mcq').modal('show');
                    }, 2000);
...
<script type="module">
      ...
      const uppy = new Uppy({ debug: true, autoProceed: false,restrictions: {maxNumberOfFiles: 1,allowedFileTypes: ['audio/*']} })
        .use(Dashboard, { trigger: '#uppyModalOpener' })        
        .use(Audio, { target: Dashboard }) 
        .use(XHRUpload, { endpoint: 'https://teacher.revisewell.com/pages/listen_audio_uploader.php', });
...
<!-- Class Selector Modal -->
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <?php 
                ...
                if (mysqli_num_rows($fetch_class_of_teacher) > 0) {
                    while($fdr = mysqli_fetch_assoc($fetch_class_of_teacher)) { ?>                       
                        <a href="/pages/listen?param=<?php echo $fdr['tree_id']; ?>&uni=<?php echo $uniq_id; ?>">
                            <button class="btn btn-sm btn-outline-primary">Grade <?php echo htmlspecialchars($fdr['rev_teacher_class'], ENT_QUOTES, "UTF-8") . ' ' . htmlspecialchars(ucfirst($fdr['rev_teacher_sec']), ENT_QUOTES, 'UTF-8') . ' ' . htmlspecialchars(ucfirst(str_replace('_', ' ', $fdr['rev_teach_subject'])), ENT_QUOTES, 'UTF-8'); ?></button>
                        </a>
                    <?php }
                }
            ?>
    ...
    <!-- create_mcq_modal Msg box -->    

<div class="modal fade" id="create_mcq" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5 activity_name" id="exampleModalLabel">ad</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <h3>To make Listen activity more interactive, Create MCQ's By clicking the below button</h3>
        <a href="<?php echo BASE_URL; ?>pages/listen_audio?uni=<?php echo $uniq_id; ?>"><button type="button" class="btn btn-primary">Create MCQ's</button></a>
      </div>
      <div class="modal-footer">
...
<div class="toast-container position-fixed bottom-0 end-0 p-3">
  <div id="liveToast" class="toast text-bg-primary" role="alert" aria-live="assertive" aria-atomic="true">
    <div class="toast-header">
      <img src="https://rev-user.s3.ap-south-1.amazonaws.com/Revisewell_logo.svg" class="rounded me-2" alt="revisewell_logo" width="75px" height="75px">
      <strong class="me-auto text-dark" style="font-size:28px;">Alert</strong>
      
      <button type="button" class="btn-close" data-bs-dismiss="toast" aria-label="Close"></button>
    </div>
    <div class="toast-body e_message">
